...
# scopes
#Equilibrio
theta_eq=np.zeros(kend)+2*np.pi/9 #40 graus
omega_eq=np.zeros(kend)+np.sqrt((m*g*np.sin(theta_eq))/Kh)
#velocidades
omega =np.zeros(kend)  # velocidade de rotacao do motor
v =np.zeros(kend)  # velocidade de rotacao do motor linearizada
#angulos
theta = np.zeros(kend)# Angulo do pendulo com a estrutura [rad]
phi = theta-theta_eq
theta_med = np.zeros(kend) # Angulo do pendulo com a estrutura
phi_med=np.zeros(kend) # Angulo do pendulo linearizado
...
for k in tqdm.tqdm((range(kend-1))):
    ...
    #SISTEMA NAO LINEAR
    sol = odeint(din_aeropendulo, [theta[k], theta_p[k]], [ Ta*k, Ta*(k+1) ], args= (omega[k],))
    theta[k+1]=sol[1,0]
