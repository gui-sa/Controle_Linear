...
theta = np.zeros(kend)-40*np.pi/180 # Angulo do pendulo com a estrutura [rad]
...
theta_ref[:]=-40 #0 [graus]
#Parametros do controlador
Kp = 500 #Só pra testar mesmo
...
for k in tqdm.tqdm((range(kend-1))):
    ...
    #theta[k] = min(theta[k], 90*np.pi/180)  # maximo 375 rad/s
    #theta[k] = max(theta[k], 0*np.pi/180)  # minimo 0
    
    
    sol = odeint(din_aeropendulo, [theta[k] + 40*np.pi/180, theta_p[k]], [ Ta*k, Ta*(k+1) ], args= (v[k],))
    theta[k+1]=sol[1,0] - 40*np.pi/180
    theta_p[k+1] = sol[1,1]
    theta_med[k+1] = theta[k+1] #+ uniform(-0.01, 0.01)
#%%======================Plotando resultado:

plt.figure()
plt.plot(np.linspace(0, Tsim,kend) , theta*180.0/np.pi+40, lw =2.0 , color="k", label = "Posicao aeropendulo")
theta_ref[-1]=theta_ref[-2]
plt.plot(np.linspace(0, Tsim,kend) , theta_ref*180.0/np.pi+40, lw =2.0 , color="r", label = "Referencia-angulo")
plt.xlabel("Tempo [s]")
